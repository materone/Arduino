...
0x81,0x85,0x99,0x01,0x09,0xE9,0xA9,0xA9,
0xF9,0xA9,0xF9,0xA9,0xA9,0xED,0x09,0x01,
0x01,0x11,0x51,0x95,0x19,0x91,0x59,0x11,
0x01,0xF1,0x11,0x09,0x0D,0x89,0x01,0x01,
...
0x00,0x00,0xFF,0x80,0x40,0x80,0xBE,0xAA,
0xAA,0xEA,0xAA,0xAA,0xBF,0x82,0x80,0x00,
0x81,0x45,0x25,0x05,0xFF,0x05,0x17,0xA5,
0x61,0x1F,0x01,0x01,0xFF,0x01,0x01,0x00,
0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
...
0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
0x00,0x00,0x01,0x00,0xC0,0x20,0x20,0x20,
0x20,0xC3,0xC0,0xC0,0x20,0x20,0x20,0x20,
0x20,0xE0,0x21,0x22,0x21,0x20,0x20,0x20,
0xC0,0xC0,0xC0,0x00,0xC3,0xC0,0xC0,0xC0,
...
void rfid()
{
  ...
  Serial.println("Begin Write IC Card");
  int incomeBits = 0 ;
  /*
  while(1){
    if ((incomeBits = Serial.available()) > 0){            // if data is available on hardwareserial port ==> data is comming from PC or notebook
      Serial.println(incomeBits);
      for(int i = 0; i<incomeBits;i++){
        char c = Serial.read();
        Serial.print(i);
        Serial.print(":[ ");
        Serial.print(c);
        Serial.print(" ]");
        int iWrites = SoftSerial.write(c);       // write it to the SoftSerial shield
        Serial.print(i);        
        Serial.print(" ");
      }
      break;
  ...
  */
  int iWrites = SoftSerial.write("88888888");       // write it to the SoftSerial shield
  Serial.print(iWrites);     
