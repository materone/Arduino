/*======================================================
Notice:
1.This is Modified edition of SoftwareSerial.* of Arduino.
2.Add 2 function to support {init SoftSerial Port} inside Setup(). 
  SoftSerial(bool inverse_logic = false);
  bool init(uint8_t rx=10,uint8_t tx=11);
3.Please do mix-use with SoftwareSerial.* of Arduino official.

Modified By: OpenThings@163.com.
LastModified:2014-09-25.
...
ISR(PCINT3_vect)
{
  SoftSerialSoftSerial::handle_interrupt();
}
#endif
...
SoftSerial::SoftSerial(uint8_t receivePin, uint8_t transmitPin, bool inverse_logic) : 
...
//Add By OpenThings@163.com,to Delay init.
SoftSerial::SoftSerial(bool inverse_logic) : 
  _rx_delay_centering(0),
  _rx_delay_intrabit(0),
  _rx_delay_stopbit(0),
  _tx_delay(0),
...
//Add By OpenThings@163.com.
bool SoftSerial::init(uint8_t rx,uint8_t tx)
{
  setRX(rx);
  setTX(tx);
  return true;
