...
//in horizon mode
void SeeedOLED::setBitmapRect(unsigned char startPage,unsigned char endPage, unsigned char startColumn, unsigned char endColumn){
	  sendCommand(0x22); 			//set page address
	  sendCommand(startPage);	//0-7 startpage
	  sendCommand(endPage);	//0-7 end page
	  sendCommand(0x21);//set column
    sendCommand(startColumn); 	//set start column 0- 127
    sendCommand(endColumn); 	//set end column 0-127
}
...
can use drawbitmap()
*/
void SeeedOLED::putHZChar(unsigned char *hzData,int len)
{
    unsigned char i=0;
    for(i=0;i<len;i++)
    {
       sendData(pgm_read_byte(&hzData[i])); 
    }
}
...
void SeeedOLED::drawBitmap(unsigned char *bitmaparray,int bytes,bool usePgmspace)
{
  char localAddressMode = addressingMode;
  if(addressingMode != HORIZONTAL_MODE)
  {
...
void SeeedOLED::setVerticalScrollProperties(bool direction,unsigned char startPage, unsigned char endPage, unsigned char scrollSpeed,unsigned char verticalOffset)
{
    /*
     Use the following defines for 'direction' :
