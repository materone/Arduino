...
static unsigned char fontDatas[8][32] PROGMEM={
  //超
  {
...
void setup()
{
  ...
  SeeedOled.drawBitmap(fontDatas[3],16);
  SeeedOled.drawBitmap(fontDatas[4],16);
  SeeedOled.drawBitmap(fontDatas[3] + 16,16);
  SeeedOled.drawBitmap(fontDatas[4] + 16,16);
}
void loop()
{
  test();
...
void test(){
  ...
  unsigned char i = 0;
  unsigned char f = 0;
  long long_num = ultrasonic.RangeInCentimeters;
  ...
  //SeeedOled.setTextXY(4,3);           //Set the cursor to 0th Page, 0th Column  
  //SeeedOled.putNumber(ultrasonic.RangeInCentimeters);           //Print number
  //serial output ,can recv and display data in pc
